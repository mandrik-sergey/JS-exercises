...
let questions = {
    ...
    'Пеликан':'У какой птицы самый большой клюв?' ,
    'Днепр':'Самая большая река Украины?',
    'Говерла': 'Самая высокая гора Украины?',
    '09.03.1998':'Когда родился Шевченко Т.Г.? ДД.ММ.ГГГГ' ,
    'Нет': 'Страусы от опасности прячут голову в песок?',
    'Может':'Молния не может ударить дважды в одно и то же место?' ,
    'Бразилия': 'Какая страна производит больше всего кофе в мире?',
    'Апельсиновый':'Какой вкус у Куантро?'
};
function quiz(obj,key,answer) {
    for (let key in obj) {
        answer = prompt(obj[key]);
    }
    if (key === answer ) {
        alert('Ваши балы');
        let balls = 0;
